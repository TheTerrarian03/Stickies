...
### bigger functions
def makeNewSticky():
  print("WARNING: Adding new files is experimental.")
  f = open("SAVEs/NewSticky.sticky", "wt")
  f.write("/TITLE New Sticky\n/THEME YELLOW\n/SHOW-NUMBERS FALSE\n/EDIT-TITLE TRUE\n/START-CONTENT\nTITLE: New Sticky\nList your problems here! :D\n-Logan Meyers\n/END-CONTENT\n")
  f.close()

def renameSticky(oldFilename, newTitle, infoObj):
  """print(oldFilename, newTitle)
  try:
    os.rename(oldFilename, "SAVES/"+newTitle+".sticky")
  except FileNotFoundError:
    pass
  infoObj.loadStickies()
  infoObj.changeActive(newTitle)"""
  f = open(oldFilename)
  oldContents = f.read()
  f.close()
...
def loadStickies():
  ...
  for File in os.listdir("SAVES/"):
    if File.endswith(".sticky"):
      stickyAttr = {"TITLE": None, "THEME": None, "SCALE": None, "SHOW-NUMBERS": None, "EDIT-TITLE": None, "CONTENT": "", "MainColor": [], "AltColor1": [], "AltColor2": [], "SAVEPATH": None, "filename": None}
      ctSt = False
      done = False
      f = open("SAVES/"+File)
      stickyAttr["filename"] = f.name
      ...
      for line in f:
        ...
        if lineS[0] == "/TITLE":
          ...
          for word in lineS[1:]:
            ...
            if word != lineS[-1]:
              title += " "
          stickyAttr["TITLE"] = title
        elif lineS[0] == "/THEME":
          stickyAttr["THEME"] = lineS[1]
        elif lineS[0] == "/SCALE":
          stickyAttr["SCALE"] = int(lineS[1])
        elif lineS[0] == "/SHOW-NUMBERS":
          if lineS[1] == "TRUE\n":
            stickyAttr["SHOW-NUMBERS"] = True
          elif lineS[1] == "FALSE\n":
            stickyAttr["SHOW-NUMBERS"] = False
        elif lineS[0] == "/EDIT-TITLE":
          if lineS[1] == "TRUE\n":
            stickyAttr["EDIT-TITLE"] = True
          elif lineS[1] == "FALSE\n":
            stickyAttr["EDIT-TITLE"] = False
